...
from config import EARLY_STOPPING, TF_FIT_VERBOSE
...
class GNN:
    ...
    def train(self, train_gen, validation_gen):
        ...
        es = EarlyStopping(
            monitor="val_loss", min_delta=0, patience=EARLY_STOPPING, restore_best_weights=True
        )
        ...
        history = self.model.fit(
            ...
            epochs=epochs, verbose=TF_FIT_VERBOSE, callbacks=[es]
        )
    ...
    def validate(self, test_gen):
        ...
        begin_time = datetime.datetime.now()
        test_metrics = self.model.evaluate(test_gen.load(), steps=test_gen.steps_per_epoch, verbose=TF_FIT_VERBOSE)
        end_time = datetime.datetime.now()
